        ...
        <h2><?php echo esc_html( $page_label . ' Settings' ); // page_label;          ?>
            <div class="tp-feature">
                ...
                <div class="tp-tooltip">
                    <p>This feature turns off WordPress’s XML-RPC functions, plugging a known vulnerability often
                        exploited in remote-access attacks.</p>
                </div>
            </div>
        </h2>
...
if ( is_array( $current_settings_fields ) && !empty( $current_settings_fields ) ) {
    foreach ( $current_settings_fields as $key => $field ) {
        ...
        echo Utility::get_template( 'settings/fields/' . sanitize_key( $field_name ) . '.php', $args );
    }
}
?>
            </div>
